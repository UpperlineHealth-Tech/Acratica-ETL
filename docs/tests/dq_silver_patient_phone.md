# Data Quality Check: dq_silver_patient_phone (ATHENA)

**Source file:** `models/data_quality/ATHENA/dq_silver_patient_phone.sql`  
**GitHub permalink:** https://github.com/UpperlineHealth-Tech/Acratica-ETL/blob/1a2168e26e17e6bf7338c1a66df79cf384927491/models/data_quality/ATHENA/dq_silver_patient_phone.sql?plain=1#L1-L48

## What this check does

This check produces a **single-row summary** of data quality for the `PHONE` field in the silver model:

- `{{ ref('silver_patient') }}`

It reports:

- how many rows have `PHONE` missing (`null_count`)
...
**Why we have it:** EMPI matching quality depends on clean, standardized demographics. If a key field like `PHONE` is frequently null/invalid, match rates and downstream outreach quality can degrade.
...
The core “validity” logic is implemented here: https://github.com/UpperlineHealth-Tech/Acratica-ETL/blob/1a2168e26e17e6bf7338c1a66df79cf384927491/models/data_quality/ATHENA/dq_silver_patient_phone.sql?plain=1#L5-L12
...
```bash
dbt build --select dq_silver_patient_phone
```
...
- `null_count`: number of rows where `PHONE` is missing
- `invalid_count`: number of rows where `PHONE` is present but fails the validity rule
