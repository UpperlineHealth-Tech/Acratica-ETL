# Data Quality Check: dq_silver_patient_city (ATHENA)

**Source file:** `models/data_quality/ATHENA/dq_silver_patient_city.sql`  
**GitHub permalink:** https://github.com/UpperlineHealth-Tech/Acratica-ETL/blob/1a2168e26e17e6bf7338c1a66df79cf384927491/models/data_quality/ATHENA/dq_silver_patient_city.sql?plain=1#L1-L24

## What this check does

This check produces a **single-row summary** of data quality for the `CITY` field in the silver model:

- `{{ ref('silver_patient') }}`

It reports:

- how many rows have `CITY` missing (`null_count`)
...
**Why we have it:** EMPI matching quality depends on clean, standardized demographics. If a key field like `CITY` is frequently null/invalid, match rates and downstream outreach quality can degrade.
...
The core “validity” logic is implemented here: https://github.com/UpperlineHealth-Tech/Acratica-ETL/blob/1a2168e26e17e6bf7338c1a66df79cf384927491/models/data_quality/ATHENA/dq_silver_patient_city.sql?plain=1#L1-L24
...
```bash
dbt build --select dq_silver_patient_city
```
...
- `null_count`: number of rows where `CITY` is missing
- `invalid_count`: number of rows where `CITY` is present but fails the validity rule
